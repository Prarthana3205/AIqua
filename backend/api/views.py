...
def find_nearest_lakes(user_lat, user_lon):
        ...
        try:
            ...
            if lake_key not in unique_lakes:  # Avoid duplicate lakes
                ...
                lakes_with_distance.append({
                    ...
                    "dissolved_oxygen": lake.get("Dissolved O2 (mg/L)", 0),
                    "ph": lake.get("pH", 0),
                    "bod": lake.get("BOD(mg/L)", 0),
                    "nitrate": lake.get("NitrateN (mg/L)", 0),
                    "total_dissolved_solids": lake.get("Total Dissolved Solids (mg/L)", 0),
                })
    ...
    # Sort lakes by distance and return the closest 10
    lakes_with_distance.sort(key=lambda x: x["distance"])
    return lakes_with_distance[:10]
